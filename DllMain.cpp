...
{
	// AIM
	g_Console.addCvar("aim_active", 0, false);
	g_Console.addCvar("aim_key", 0, false);
	...
	g_Console.addCvar("vis_chams", 0, false);
...
bool IsPistol(CBaseEntity* pEntity){
	...
	case WEAPON_USP_SILENCER:{return false; break; }
...
char* GetWeaponName(CBaseEntity *pEntity)
{
	...
	case WEAPON_HKP2000:{strcpy(m_szPrintName, "P228"); break; }
	case WEAPON_USP_SILENCER:{strcpy(m_szPrintName, "USP"); break; }
...
void __stdcall hkdPaintTraverse( unsigned vguiPanel, bool forceRepaint, bool allowForce)
{
	...
	g_Draw.DrawStringA(g_Draw.m_Font, false, 5, 11, 255, 255, 255, 255, "pCheat Internal");
	...
	for(INT ax = 0;ax < tSize;ax++)
	{
		...
		if(g_Aimbot.isValidPlayer(pBaseEntity))
		{
				...
				WorldToScreen(vHitbox, vHitboxScreen))
			{
				...
				if (g_Console.getValue("esp_enemyonly") == 1) //enemy only esp
				{
...
void InitDllThread ( void* ptr )
{
	...
	dwOriginCreateMove = g_pCreateMoveVMT->dwGetMethodAddress(24);
	g_pCreateMoveVMT->dwHookMethod((DWORD)nakedCreateMove, 24);
