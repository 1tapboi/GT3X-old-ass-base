...
VOID CDraw::init(int size)
{
	...
	g_pSurface->SetFontGlyphSet(m_Font,"Tahoma",10 , FW_NORMAL,0,0,0x200);

	font_GUI = g_pSurface->CreateFont();
	g_pSurface->SetFontGlyphSet(font_GUI, "Tahoma", 15, FW_NORMAL, 0, 0, 0);

	font_GUI2 = g_pSurface->CreateFont();
	g_pSurface->SetFontGlyphSet(font_GUI2, "Tahoma", 11, FW_NORMAL, 0, 0, 0x210);

	font_Tab = g_pSurface->CreateFont();
	g_pSurface->SetFontGlyphSet(font_Tab, "Tahoma", 15, FW_NORMAL, 0, 0, 0x200);

	font_menu = g_pSurface->CreateFont();
	g_pSurface->SetFontGlyphSet(font_menu, "Tahome", 28, 500, 0, 0, 0x200);

	font_build = g_pSurface->CreateFont();
	g_pSurface->SetFontGlyphSet(font_build, "Tahoma", 13, FW_NORMAL, 0, 0, 0x210);

	Font_CHECK = g_pSurface->CreateFont();
	g_pSurface->SetFontGlyphSet(Font_CHECK, "Tahoma", 17, FW_NORMAL, 0, 0, 0x210);
}
